...
void selfSeed_Field(Domain *D)
{
   ...
   for(h=0; h<1; h++) 
     for(i=startI; i<endI; i++) {
       //shiftT=(minI+i-startI-sliceN*0.5)*dt + delayT;
       shiftT=(minI+i-startI)*dt;
       for(j=0; j<N; j++) {
         compVal=D->U[h][i][j];
		  
         for(nn=0; nn<sliceN; nn++) {
           tau = nn*dt + shiftT;
           ctau=velocityC*tau;
