...
//if(myrank==0) printf("myrank=%d,iteration=%d, calculate_twiss is done\n",myrank,iteration);

      // updating wake_field
      if(iteration%D.wakeFieldStep==0) updateWakeField(&D,iteration); else ;
//if(myrank==0) printf("myrank=%d,iteration=%d, updateWakeField is done\n",myrank,iteration);

      // update total energy
      updateTotalEnergy(&D,iteration);
//if(myrank==0) printf("myrank=%d,iteration=%d, updateTotalEnergy is done\n",myrank,iteration);
      updateBFactor(&D,iteration);		
//if(myrank==0) printf("myrank=%d,iteration=%d, updateBFactor is done\n",myrank,iteration);
...
//if(myrank==0) printf("myrank=%d,iteration=%d, solveField is done\n",myrank,iteration);

		//Chicane
      chicane_test(&D,iteration);
//if(myrank==0) printf("myrank=%d,iteration=%d, chicane_test is done\n",myrank,iteration);
